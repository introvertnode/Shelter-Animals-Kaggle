...
clean <- function(df){
  ...
  # check most popular breeds and if an animal is of a popular breed
  popularBreeds <- names(summary(as.factor(dt$Breed),maxsum=20L))
  dt[!(Breed %in% popularBreeds), popularBreed := 0]
  dt[Breed %in% popularBreeds, popularBreed := 1]
  dt$popularBreed <- as.factor(dt$popularBreed) 
